...
export default function InventarioPage() {
    ...
    useEffect(() => {
        ...
        // Filtro de tipo
        if (tipoFiltro === "pesable") {
            filtered = filtered.filter(p => p.es_pesable)
        } else if (tipoFiltro === "unitario") {
            filtered = filtered.filter(p => !p.es_pesable)
        }

        // Filtro de stock
        if (stockFiltro === "bajo") {
            filtered = filtered.filter(p => !p.es_pesable && p.stock_cantidad < p.stock_minimo && p.stock_cantidad > 0)
        } else if (stockFiltro === "sin_stock") {
            filtered = filtered.filter(p => !p.es_pesable && p.stock_cantidad === 0)
        } else if (stockFiltro === "ok") {
            filtered = filtered.filter(p => !p.es_pesable && p.stock_cantidad >= p.stock_minimo)
        }
    ...
    const productosPesables = filteredProductos.filter(p => p.es_pesable).length
    const stockBajo = filteredProductos.filter(p => !p.es_pesable && p.stock_cantidad < p.stock_minimo && p.stock_cantidad > 0).length
    const sinStock = filteredProductos.filter(p => !p.es_pesable && p.stock_cantidad === 0).length
                ...
                <Card>
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold text-green-600">
                            {filteredProductos.filter(p => !p.es_pesable && p.stock_cantidad >= p.stock_minimo).length}
                        </div>
                        <p className="text-xs text-muted-foreground mt-1">
                            Stock suficiente
                        </p>
                        ...
                        <div className="flex-1 min-w-[200px]">
                            ...
                            <Select value={tipoFiltro} onValueChange={setTipoFiltro}>
                                ...
                                <SelectContent>
                                    ...
                                    <SelectItem value="unitario">Unitarios</SelectItem>
                                    <SelectItem value="pesable">Pesables</SelectItem>
                                </SelectContent>
                            </Select>
                        </div>
                        ...
                        <TableBody>
                            {loading ? (
                                ...
                                currentProductos.map((producto) => {
                                    const stockBajo = !producto.es_pesable && producto.stock_cantidad < producto.stock_minimo && producto.stock_cantidad > 0
                                    const sinStock = !producto.es_pesable && producto.stock_cantidad === 0
                                            ...
                                            <TableCell className="text-right">
                                                {producto.es_pesable ? (
                                                    ...
                                                    <span className={sinStock ? "text-red-600 font-bold" : stockBajo ? "text-orange-600 font-bold" : ""}>
                                                        {producto.stock_cantidad.toFixed(0)} uds
                                                    </span>
                                                )}
                                            </TableCell>
                                            <TableCell>
                                                <Badge variant={producto.es_pesable ? "default" : "secondary"}>
                                                    {producto.es_pesable ? "Pesable" : "Unitario"}
                                                </Badge>
                                            </TableCell>
                                            <TableCell>
                                                {producto.activo ? (
