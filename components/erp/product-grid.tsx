...
export function ProductGrid({ products, categories, loading, onAddToCart }: ProductGridProps) {
    ...
    const filteredProducts = products.filter(p => {
        ...
        const aOutOfStock = a.stock_actual <= 0
        const bOutOfStock = b.stock_actual <= 0
                            ...
                            return (
                                <Card
                                    ...
                                    className={cn(
                                        ...
                                        isOutOfStock && "opacity-60 grayscale-[0.5] hover:opacity-100"
                                    )}
                                        ...
                                        {viewMode === "grid" ? (
                                            <div className="flex flex-col gap-2">
                                                <div className="flex justify-between items-start gap-1">
                                                    ...
                                                    {isOutOfStock && (
                                                        <Badge variant="destructive" className="text-[10px] px-1 py-0 h-4 leading-none shrink-0 uppercase tracking-wider">Agotado</Badge>
                                                    )}
                                                </div>
                                            ...
                                            <>
                                                <div className="flex flex-col">
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        {isOutOfStock && (
                                                            <Badge variant="destructive" className="text-[10px] px-1.5 py-0 h-4 leading-none uppercase tracking-wider">Agotado</Badge>
                                                        )}
                                                    </div>
                                                    <span className={cn(
                                                        "text-muted-foreground text-xs",
                                                        isOutOfStock && "text-destructive/80 font-medium"
                                                    )}>
