...
## 🚀 POS Panadería Stack Integration (Next.js 16, React 19, Supabase SSR)

When applying these guidelines, ALWAYS adhere to the following stack-specific rules:

1. **Next.js 16 App Router & Server Components (Default)**
   - Use React Server Components to minimize client JS bundles and improve performance.
   - Restrict `"use client"` to files that strictly require hooks, state, or browser APIs.
   - Use Next.js `"use cache"`, `cacheTag()`, and `cacheLife()` directives over unstable APIs.
   - Handle heavy rendering on the server and wrap client components in `<Suspense>`.
   
2. **Supabase SSR Authenticated Data Fetching**
   - Use `@supabase/ssr` for all database calls. Never use `@supabase/auth-helpers-nextjs`.
   - Implement `proxy.ts` session refresh pattern correctly to secure API routes.
   - Server-side data fetching MUST be asynchronous (`await params`, `await cookies()`).

3. **Modern Styling & UI Components**
   - Use Tailwind CSS v4 patterns.
   - Leverage Shadcn UI primitives for built-in accessibility (A11y/ARIA) out of the box. Ensure forms use proper Zod validation.

4. **SEO & Metadata**
   - Use Next.js 16 `export const metadata` or `export async function generateMetadata()` for SEO optimization natively in Server Components.
